...
import { getOperationById, listOperations } from "@/lib/services/operations";
...
export default async function EditOperationPage({
  ...
  if (!user) {
    redirect(`/login?redirectTo=/dashboard/operations/${id}/edit`);
  }
  ...
  let operation: Awaited<ReturnType<typeof getOperationById>> | null = null;
  let operationError = "";
  try {
    operation = await getOperationById(supabase, id);
  } catch (e) {
    operationError = e instanceof Error ? e.message : "Failed to load operation.";
  }

  const selectedWorkOrderId =
    workOrderId || operation?.work_order_id || workOrders[0]?.id || "";
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-tight">Edit operation</h1>
          <p className="mt-2 text-sm text-black/60 dark:text-white/60">
            Adjust resources, timing, and dependencies.
          </p>
        </div>
        <Link
          ...
          <form action={saveOperationAction} className="mt-4 space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block">
                ...
                <input
                  ...
                  name="sequence"
                  inputMode="numeric"
                  defaultValue={operation?.sequence ?? 1}
                  disabled={!selectedWorkOrderId || Boolean(anyLoadError)}
                />
              </label>
              <label className="block">
                ...
                <input
                  ...
                  name="quantity"
                  inputMode="decimal"
                  defaultValue={operation?.quantity ?? 1}
                  disabled={!selectedWorkOrderId || Boolean(anyLoadError)}
                />
              </label>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block">
                ...
                <select
                  ...
                  name="priority"
                  defaultValue={String(operation?.priority ?? 3)}
                  disabled={!selectedWorkOrderId || Boolean(anyLoadError)}
                >
              ...
              <label className="block">
                ...
                <select
                  className="mt-2 w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm text-black outline-none focus:border-indigo-500/40 focus:ring-4 focus:ring-indigo-500/10 dark:border-white/10 dark:bg-black/40 dark:text-white"
                  name="status"
                  defaultValue={operation?.status ?? "pending"}
                  disabled={!selectedWorkOrderId || Boolean(anyLoadError)}
                >
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block">
                ...
                <DateTimeInput
                  className="mt-2 w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm text-black outline-none focus:border-indigo-500/40 focus:ring-4 focus:ring-indigo-500/10 dark:border-white/10 dark:bg-black/40 dark:text-white"
                  name="deadline"
                  defaultValue={formatDateTimeLocal(operation?.deadline ?? null)}
                  disabled={!selectedWorkOrderId || Boolean(anyLoadError)}
                />
              </label>
              <label className="block">
                ...
                <DateTimeInput
                  className="mt-2 w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm text-black outline-none focus:border-indigo-500/40 focus:ring-4 focus:ring-indigo-500/10 dark:border-white/10 dark:bg-black/40 dark:text-white"
                  name="plannedStart"
                  defaultValue={formatDateTimeLocal(operation?.planned_start ?? null)}
                  disabled={!selectedWorkOrderId || Boolean(anyLoadError)}
                />
              </label>
            </div>
            ...
            <label className="block">
              ...
              <DateTimeInput
                className="mt-2 w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm text-black outline-none focus:border-indigo-500/40 focus:ring-4 focus:ring-indigo-500/10 dark:border-white/10 dark:bg-black/40 dark:text-white"
                name="plannedEnd"
                defaultValue={formatDateTimeLocal(operation?.planned_end ?? null)}
                disabled={!selectedWorkOrderId || Boolean(anyLoadError)}
              />
            </label>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <label className="block">
                ...
                <select
                  ...
                  defaultValue={operation?.production_line_id ?? selectedProductionLineId}
                  disabled={!selectedWorkOrderId || Boolean(anyLoadError)}
                >
                  <option value="">—</option>
              ...
              <label className="block">
                ...
                <select
                  className="mt-2 w-full rounded-2xl border border-black/10 bg-white px-3 py-3 text-sm text-black outline-none focus:border-indigo-500/40 focus:ring-4 focus:ring-indigo-500/10 dark:border-white/10 dark:bg-black/40 dark:text-white"
                  name="machineId"
                  defaultValue={operation?.machine_id ?? ""}
                  disabled={!selectedWorkOrderId || Boolean(anyLoadError)}
                >
                  <option value="">—</option>
              ...
              <label className="block">
                ...
                <select
                  className="mt-2 w-full rounded-2xl border border-black/10 bg-white px-3 py-3 text-sm text-black outline-none focus:border-indigo-500/40 focus:ring-4 focus:ring-indigo-500/10 dark:border-white/10 dark:bg-black/40 dark:text-white"
                  name="workerId"
                  defaultValue={operation?.worker_id ?? ""}
                  disabled={!selectedWorkOrderId || Boolean(anyLoadError)}
                >
                  <option value="">—</option>
                  {workers.map((worker) => (
            ...
            <label className="block">
              ...
              <div className="mt-2 max-h-40 overflow-y-auto rounded-2xl border border-black/10 bg-white/70 p-3 text-sm dark:border-white/10 dark:bg-black/30">
                {operations.length ? (
                  operations.map((op) => {
                    const isSelf = op.id === operation?.id;
                    const isChecked = operation?.dependency_ids?.includes(op.id);
                    return (
                      <label
                        key={op.id}
                        className={[
