...
export default async function NewWorkOrderPage({
  ...
  if (!user) {
    redirect("/login?redirectTo=/dashboard/work-orders/new");
  }
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-tight">Create work order</h1>
          <p className="mt-2 text-sm text-black/60 dark:text-white/60">
            Define priority, deadline, and dependencies before scheduling.
          </p>
        </div>
        <Link
              ...
              organizations.map((o) => {
                ...
                return (
                  <Link
                    ...
                    href={`/dashboard/work-orders/new?organizationId=${encodeURIComponent(o.id)}`}
              ...
              <div className="mt-2 max-h-48 overflow-y-auto rounded-2xl border border-black/10 bg-white/70 p-3 text-sm dark:border-white/10 dark:bg-black/30">
                {workOrders.length ? (
                  workOrders.map((w) => (
                    ...
                    >
                      <input type="checkbox" name="dependencyIds" value={w.id} />
